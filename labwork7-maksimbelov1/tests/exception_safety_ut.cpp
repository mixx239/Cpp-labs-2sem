...
/*
    В тесте складывается 5 инстансов класса SomeObj2 в std::list.
...
TEST_F(ExceptionSafetyTest, failesInsert) {
    TestAllocator<SomeObj2> allocator;
    unrolled_list<SomeObj2, 10, TestAllocator<SomeObj2>> unrolled_list(allocator);

    unrolled_list.push_back(SomeObj2{});
    ASSERT_EQ(unrolled_list.size(), 1);

    ASSERT_ANY_THROW(
        unrolled_list.insert(unrolled_list.begin(), 10, SomeObj2{});
    );
