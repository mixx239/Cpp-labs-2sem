# Лабораторная работа 8 - адаптеры STL

Проект реализует набор адаптеров (`Dir`, `Filter`, `OpenFiles`, `Split`, `Transform`, `AggregateByKey`, `Out` и др.) для построения конвейеров обработки данных над файлами и контейнерами

## Сборка

```bash
cd labwork8-maksimbelov1
...
## Пример запуска
В main.cpp приведен пример использования библиотеки - программа считает частоту слов во всех `.txt`‑файлах указанной директории и печатает статистику в стандартный вывод.


Создадим директорию с текстами, например:

```bash
mkdir -p sample_texts
echo "Hello world"      > sample_texts/a.txt
echo "Hello C++ world" > sample_texts/b.txt
```

Запуск подсчёта частоты слов по этой директории:

```bash
./build/labwork8 ./sample_texts
```

## Пример вывода

Для примера выше вывод будет выглядеть так:
